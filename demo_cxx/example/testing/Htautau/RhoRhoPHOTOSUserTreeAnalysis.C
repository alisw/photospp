...
int RhoRhoPHOTOSUserTreeAnalysis(HEPParticle *mother,HEPParticleList *stableDaughters, int nparams, double *params)
{
	...
	cm_e+=part->GetE();
      ...
      switch(part->GetPDGId()){
         ...
         case 211:
	   d_pi_plus.Set(&part->GetP4());
	   d_pi_plus.SetM(part->GetM());   
	   break;
         case -211:
	   d_pi_minus.Set(&part->GetP4());
	   ...
	   if(d_pi0_minus.GetX1()==0&&d_pi0_minus.GetX2()==0&&d_pi0_minus.GetX3()==0){
	     d_pi0_minus.Set(&part->GetP4());
	     d_pi0_minus.SetM(part->GetM());
	   }
	   else{
	     d_pi0_plus.Set(&part->GetP4());
    ...
    if(photon_e==0){
      if(y1*y2>0)
	fillUserHisto("acoplanarity-angle-C-no-photon",theta,1.0,0,2*M_PI);
      else
	fillUserHisto("acoplanarity-angle-D-no-photon",theta,1.0,0,2*M_PI);

    }
    if(photon_e>0&&photon_e<1.0){
      if(y1*y2>0)
	fillUserHisto("acoplanarity-angle-C-photon-up-to-1-GeV",theta,1.0,0,2*M_PI);
      else
	fillUserHisto("acoplanarity-angle-D-photon-up-to-1-GeV",theta,1.0,0,2*M_PI);
    }
    if(photon_e>1.0){
      if(y1*y2>0)
	fillUserHisto("acoplanarity-angle-C-photon-over-1-GeV",theta,1.0,0,2*M_PI);
      else
	fillUserHisto("acoplanarity-angle-D-photon-over-1-GeV",theta,1.0,0,2*M_PI);
    }
