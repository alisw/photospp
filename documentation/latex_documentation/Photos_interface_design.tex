...
The {\tt qedrad[NMXHEP]} is used in the algorithm of pair emission.
    ...
    {\bf ZbW: At present it is Not used. TP: maybe we should remove it altogether? ZbW: Yes we should do that.}
    \end{description}
\end{description}
...
photons or lepton pairs. This part of the code was not tested to large detail.
Our tests were performed for the cases where modified decay was the last one of the event record, thus  shifting of consecutives entries was not necessary.
{\bf ZbW: really? we should check that.}
...
\emph{If the decays $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e$ or
      ...
      So, for example, $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e \gamma$ will be skipped as well. It is important to realize that exluding 
$\tau^- \to \rightarrow \nu_\tau \pi^-$ lead to exclusion of
$\tau^- \to \rightarrow \nu_\tau \pi^-\pi^0$ as well. If it is not required it must be allowed separately. {\bf  ZBW last sentence must be verified.}}. %TP: I don't think I explain it clearly... it should probably be redone.
...
{\bf ZbW I do not understand, looks like the printed message is `Debug(703) for example. }
\begin{itemize}
