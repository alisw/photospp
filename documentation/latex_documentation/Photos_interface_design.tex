...
Until recently HEPEVT was used as structure for 
comunication between physics Monte Carlo programs and Detector/reconstruction 
packages. Experimental physicists were using that structure  as well.

Recently, because of gain in flexibility FORTRAN is often replaced by C++ and 
instead of HEPEVT   C++ event structure called HepMC  \cite{Dobbs:2001ck}
is used. Nothing prevents 
moving PHOTOS to environment based on HepMC and revrite whole (or at first 
event record dependent parts) 
...
At present, such migration is convenient for the users, because they can work
...
Channel dependent matrix elements of PHOTOS, at present available only 
