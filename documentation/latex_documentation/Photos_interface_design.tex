...
{\tt PHOTOS} Monte Carlo was established at  0.3\% or even   0.2\% for thecase of matrix element correction activated
and for all of final state radiative corrections and not only photonic bremsstrahlung.}
...
\begin{itemize}
  ...
  \item {\tt Photos::maxWtInterference(double interference)} \\
        Set the maximum interference weight. The default 2 is adopted to decays when at most two charged decay products
        are present\footnote{For 
        the decays like $J/\psi \to 5\pi^+ 5\pi^-$ higher value, corresponding to number of charged 
        products must be set. This will slow the algorithm linearly with the maximum interference weight only,  all 
