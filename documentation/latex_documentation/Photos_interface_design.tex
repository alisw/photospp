...
\begin{itemize}
  ...
  \item {\bf Event Record Interface} \\
       ...
       through the classes {\tt PhotosHEPEVTEvent} and {\tt PhotosHEPEVTParticle}.
...
\begin{itemize}
  ...
  \item {\tt Photos::forceMass(int pdgid, double mass)} \\
        ...
        from 4-vector.
        Only used if {\tt forceMassFrom4Vector = true} (default).
  \item {\tt Photos::forceMassFromEventRecord(int pdgid)} \\
        When particles with PDGID and -PDGID will be processed by Photos,
        their mass value will be taken from event record instead of being
        calculated from 4-vector.
        Only used if {\tt forceMassFrom4Vector = true} (default).
  \item {\tt Photos::createHistoryEntries(bool flag)} \\
        When flag is set to {\tt true}, if event record allows it,
        {\tt Photos} will create history entries of particles
        ...
        photon was added. History entries have {\tt status = 3} and are added
