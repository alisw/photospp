...
distribution. {\bf ZbW may be higher versions as well?}
...
\begin{itemize}
  \item {\bf {\tt photos-C} }\\
       This part of the code includes numerical algorithms of {\tt PHOTOS},
       as well as  an interface 
       to this {\tt C} numerical library of {\tt PHOTOS}. In particular,
	    routine which invokes the processing
	   of a single branch and the actual code for its processing.
{\bf ZbW: something must be corrected here.} 
       The most important part of this module, is the {\tt PH\_HEPEVT\_Interface\_} class.
        This class is responsible for writing
	   the decay branch to be processed into the internal event record struct {\tt hep}.
       For further details regarding the interface to {\tt C} structs
	    and methods see Appendix \ref{Interface to PHOTOS}.
...
objects {\tt hep} and {\tt pho}. In FORTRAN these were common blocks PH\_HEPEVT and PHOEVT.
The qedrad[NMXHEP] is used in algorithm of pair emission.
    \begin{description}
    \item[QEDRAD[10000]]  \textit{bool} flag enabling a {\tt FORTRAN} users
    to block emission from particles stored in PH\_HEPEVT.
    {\bf ZbW: At present it is Not used.}
    \end{description}
\end{description}
...
\item[PHOTOS\_MAKE\_C]  like {\tt PHOTOS\_MAKE} of {\tt FORTRAN} part of interface, but now in C. \\
  ...
  Parameters:
  \begin{enumerate}
    \item \textit {int id} ID of particle from which {\tt PHOTOS} starts processing. In the {\tt C++} case the importance of this parameter is limited as only one branch is in {\tt hep} struct at a time. {\bf ZbW or pho?}
  \end{enumerate}
\end{description}
...
\begin{itemize}
  ...
  \item {\tt PYTHIA 8.1} \cite{Sjostrand:2007gs} or later. {\tt PYTHIA 8.1} must be compiled with {\tt HepMC} 2.xx
        ...
        {\bf ZbW: Note restrictions for our examples due to change of initializations and makefile-s from PYTHIA 8.200 on}
...
linked. {\bf ZbW: what about HEPEVT example?}
...
{\tt ./configure}. For details see {\tt README} in the {\tt PHOTOS} main directory. {\bf ZbW: still true?}
...
\begin{itemize}
 ...
 \item {\tt Photos::suppressBremForDecay(0, motherID)} \hfill \\
	   ...
	   If   set  to zero all decay channels are skpied and not only of a given
           multiplicity. {\bf ZbW: I do not understand role of first paprameter equal 0, it is implicitly given below. The above sentence may be incorrect.}
...
\emph{If the decays $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e$ or
	  ...
	  and the mother PDGID.} {\bf Z bW: what about $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e\gamma$ ?} \\ \\
...
At present only the following debug messages can be printed {\bf ZbW I do not understand, may be just `with' missing}:
\begin{itemize}
...
\begin{itemize}
  ...
  \item {\tt Photos::setMeCorrectionWtForW(bool corr)} \\
        ...
        This option will be elaborated further. {\bf ZbW: I do not understand restriction.}
