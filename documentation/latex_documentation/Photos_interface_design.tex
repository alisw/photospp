...
in publications as well as in the code, now in C++. \\
  ...
  \begin{itemize}
    \item { \bf struct\_decl.h } - contains declaration of structures and functions defined in other files within this folder. %TP: I just realized we don't have photos-C.h file. We can rename this file to photos-C.h and remove this entry from here
                                   {\tt Photos} class uses these structs to set several options.
    ...
    \item { \bf forW-MEc.cxx } - routines to calculate matrix element in W decay, own programming style.
    \item { \bf forZ-MEc.cxx } - routines to calculate matrix element in Z decay, own programming style. %TP: what does 'own programming style' mean?
...
\begin{itemize}
  ...
  \item {\tt PYTHIA 8.1} \cite{Sjostrand:2007gs} or later. {\tt PYTHIA 8.1} must be compiled with {\tt HepMC} 2.xx
        ...
        {\bf Note restrictions for our examples due to change of initializations and makefile-s from PYTHIA 8.200 on}
...
linked. {\bf what about HEPEVT example?}
...
\begin{itemize}
 ...
 \item {\tt Photos::suppressBremForDecay(0, motherID)} \hfill \\
	   ...
	   If   set  to zero all decay channels are skpien and not only of given
           multiplicity. {\bf I do not understand role of first paprameter equal 0}
...
At present only the following debug messages can be printed {\bf I do not understand}:
\begin{itemize}
...
\begin{itemize}
  ...
  \item {\tt Photos::setMeCorrectionWtForW(bool corr)} \\
        ...
        This option will be elaborated further. {\bf I do not understand restriction.}
