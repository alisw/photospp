...
This is quite convenient for tracing problems in the
...
by other generator, and {\tt PHOTOS} should not be activated on such a decay.
...
results can be easily compared\footnote{We thank Andy Buckley for checking numerically
 that our conclusions on first order exact YFS exponentiation resluts extend
...
at 0.2 \% level. 
On the other hand, for observalbes were two hard photons were requested
...
than programs based on first order matrix element and exponentiation.
This conclusion need to be investigated if   
...
for photon emission can not be controlled by theory sufficiently well
(scalar QED may 
...
do not guarrantee that the program will perform well on a particular platform and installation. Tests and debugging of installation
...
may be useful. For that purpose  steering of monitoring\footnote{See section \ref{App:Logging}
...
the authors and help to identify faster the cause of the problems.


The next step in benchmarking relies on comparisons with benchmark distributions. 
At present, we store these tests with the help of ROOT \cite{Antcheva:2009zz} and our {\tt MC-TESTER} \cite{Davidson:2008ma}.
...
exhibit properties of unphysical infrared regulators (ee section 6.1 of 
...
bremsstrahlung  effect is similar to the one of eg. Z polarization. In  fig~\ref{fig:KKMC} this result is reproduced.
...
In the right plot, bremsstrahlung from $Z$ and  $\tau^-$ decays is
taken into account.
...
only events
  ...
  in the $H$ rest frameare taken.  For the definition of selections C
  ...
  recreated by  the test located in the {\tt
    ...
    examples/testing/README-plots} for technical details. Production process is generated with {\tt PYTHIA} 8.1.
...
non-QED corrections can be in priciple installed into the {\tt PYTHIA 8.1} - {\tt PHOTOS} simulation with 
the help of the e.g. {\tt TAUOLA} interface \cite{Davidson:2010rw}.
But such solution to be precise require further work \cite{Bardin-private}.
...
direct application to physics. As explained in the caption the LHC production process was used.
\begin{figure}[h!]
\centering
...
The plots have been created using a custom {\tt UserTreeAnalysis} of {\tt MC-TESTER}.
They can be recreated by  test located in the {\tt examples/testing/Zee} directory, see  {\tt examples/testing/README-plots} for technical details.
\label{fig:lineshape}
}
\end{figure}
...
final state particles with the second mother, in contrary to choice of {\tt LC-analysis} of  Ref.~\cite{Golonka:2002rz} } Root files for 
...
They areof limited but noneteless  of  some physics interest as well.
