...
{\bf TEMPORARY PROBLEM with PYTHIA STATUS CODES. something got changed between pythia version 8.100 and 8.135. As consequence status code for 
otherwise nicely decaying particles is not anymore 2. We have introduced patch for that in file PhotosEvent.cxx of temporary nature. After patch 
program should work with older version as well, but we are not sure if there are no negative consequences. }
...
\subsection{{\bf Use of {\tt processParticle} and {\tt processBranch} methods (method under construction)}}
\label{PHOTOSgun}
