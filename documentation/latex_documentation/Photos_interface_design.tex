...
For a long time, the {\tt PHOTOS} Monte Carlo \cite{Barberio:1990ms,Barberio:1993qi} 
...
in {\bf src/photos-C/ } directory.
 ...
 with the {\tt FORTRAN} version is kept. The appropriate descriptions remain valid;
in publications as well as in the code, now in {\tt C++}. \\
  ...
  \begin{itemize}
    ...
    \item { \bf forW-MEc.cxx } - routines to calculate matrix element in $W$ decay. 
                                 Note distinct programming style from the rest of code in {\tt photos-C} directory.
...
Examples can be adapted to use {\tt pythia8.1}. The necessary changes are explained in {\tt examples/README-PYTHIA-VERSIONS}.}.
