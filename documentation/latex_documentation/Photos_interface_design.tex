...
and to rewrite the whole 
 {\tt PHOTOS}\footnote{An up-to-date version of the code described in this paper is
available from the web page of our project~\cite{photosC++}. 
  }
 to {\tt C++}. In fact implementation of the algorithm in that language 
...
and the {\tt TAUOLA} interface \cite{Davidson:2010rw}, previously ported to {\tt HepMC} and {\tt C++}.
...
Respectively, they describe the interface to the old { FORTRAN} style part of the code which is now written in C,
...
limited and was postponed to a later step of the project development.
On the other hand, there were no obstacles for such a transformation to be
...
For calculation of matrix element kernels, mother mothers are needed as well. 
...
Prior to activation of the algorithm for  photon(s) (lepton pairs) generation and kinematic construction,
...
construction is rewritten from   {\tt FORTRAN} to C. It is extended
in the last few years and   feature the options presented above.
...
\subsection{{\tt C++} and {\tt HepMC} Specific Requirements}
...
In fact, the program can use as an input old {\tt FORTRAN} event record
{\tt HEPEVT} as an input as well, variant of interface is prepared 
for that purpose, but it is less comonly used, thus it is less tested.
...
record, which is used for the {\tt C++} based interface, has to be taken into
...
\begin{itemize}
  ...
  \item \textbf{Status codes:} only information on whether a given particle is incoming, outgoing or intermediate will be used. We assume the codes used will be 0, 1 or 2 like in {\tt HEPEVT}. All other codes will be treated as
...
We can decide on alternative bar code conventions. Our programs, {\tt TAUOLA} and
...
was rewritten to esentially  plain  {\tt C}. Common blocks were replaced
...
called {\tt hep}  (in older versions it was FORTRAN {\tt PH$\_$HEPEVT} common block);
it is using {\tt HEPEVT},
the event record of {\tt FORTRAN} as specification for struct type definition.
...
\item Each branching  is processed by {\tt PHOTOS} separately:
	...
	\item The {\tt PHOTOS} photon adding algorithm is invoked 
 	\item The resulting branching is taken back from {\tt hep} and any changes made by {\tt PHOTOS}
	      to the already existing particles and their whole decay trees are integrated into the event record.
	\item Finally, the created photons, or leptons,  are added to the event record.
	\end{enumerate}
...
all the functionality of its {\tt FORTRAN} predecessor to be available for the {\tt HepMC} data
structure. Some new methods for improved initialization are introduced. The new program 
...
In this paper we  present an algorithm, in its  
 version which is compatible with the one implemented in {\tt FORTRAN}.
...
\item
Methods devised to check the content of {\tt HepMC} as used by {\tt PHOTOS} are
...
{\bf ZbW: Do we have this option? TP: No, but we can process qq->ll without intermediate Z.}
...
{\tt ROOT} files for {\tt KKMC CEEX} samples are, however, created outside of the distribution.
  \label{fig:gamgam}
}
\end{figure}
...
event records is now rewritten into {\tt C++} and uses the {\tt HepMC} event record 
format for input and output. Interface to {\tt HEPEVT} event record of {\tt FORTRAN} is provided.
...
into the version, now fully in C, of internal algorithms of {\tt PHOTOS}. The necessary information is extracted
...
alone, even in case when pair emission is not taken into account.
}
...
the  {\tt C++} definition style is adapted, small letters are for structs and variable names.
...
Internally in {\tt C++}  {\tt Photos} code, the {\tt HEPEVT}-like  data type
...
from some particles. This struct is not preserved in {\tt C++}.
 Instead the qedrad[NMXHEP] field is added to {\tt HEPEVT} struct type for two 
objects {\tt hep} and {\tt pho}. In FORTRAN these were common blocks {\tt PH\_HEPEVT} and {\tt PHOEVT}.
The {\tt qedrad[NMXHEP]} is used in the algorithm of pair emission.
    \begin{description}
    \item[QEDRAD[10000]]  \textit{int} flag enabling a {\tt FORTRAN} users %TP: we should probably change it to bool in code
    to block emission from particles stored in {\tt PH\_HEPEVT}.
    {\bf ZbW: At present it is Not used. TP: maybe we should remove it altogether?}
    \end{description}
\end{description}
...
\begin{description}
\item[PHOTOS\_MAKE\_C]  like {\tt PHOTOS\_MAKE} of {\tt FORTRAN} part of interface, but now in C. \\
  Return type: \textit{void} \\
  Parameters:
  \begin{enumerate}
    \item \textit {int id} ID of particle from which {\tt PHOTOS} starts processing. In the {\tt C++} case the importance of this parameter is limited as only one branch is in {\tt hep} struct at a time. {\bf ZbW or pho? TP: Both. Only one branch is copied from event record to hep and then internally by PHOIN, from hep to pho.}
  \end{enumerate}
\end{description}
...
\begin{itemize}
  ...
  \item {\tt PYTHIA 8.1} \cite{Sjostrand:2007gs} or later. {\tt PYTHIA 8.1} must be compiled with {\tt HepMC} 2.xx
        ...
        {\bf ZbW: Note restrictions for our examples due to change of initializations and makefile-s from PYTHIA 8.180 on.
             TP: We still have not decided which versions we want to support. There are three setups: 8.170, 8.180 and 8.200.
                 We have the first setup, second is easy, third a bit more complicated, but only a bit. We could support all but this complicates makefiles...}
...
If neither {\tt HepMC}, {\tt Pythia8} nor {\tt MC-TESTER} are accessible, 
...
{\tt ./configure}. For details see {\tt README} in the {\tt PHOTOS} main directory. {\bf ZbW: still true? TP: yes, however, it uses pythia8.135. Uf we change pythia support, we have to update these config files!}
...
\begin{itemize}
 ...
 \item {\tt Photos::suppressBremForDecay(0, motherID)} \hfill \\
       When only the PDGID of the mother is provided, (the {\it daughterCount} is 0) {\tt PHOTOS} will skip all decay channels
	   of this particle. 
           {\bf ZbW: I do not understand role of first paprameter equal 0, it is implicitly given below. The above sentence may be incorrect.
           TP: Is it a bit more clear now? This number does not indicate multiplicity but simply number of daughters.
               Also, below is different function but with the same two use cases.}
 \item {\tt Photos::suppressBremForBranch(daughterCount, motherID, d1ID, d2ID, ...)} \hfill \\
       ...
       The usage of this function is similar to the two uses of the previous function. The difference is
...
\emph{If the decays $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e$ or
      $\tau^+ \rightarrow \bar \nu_\tau \pi^+$ are found, they will be skipped by {\tt PHOTOS}{\footnote
      Note that the first line of this example states that any decays of $\tau^-$
      that contain $\nu_\tau$, $e^-$ and $\bar \nu_e$ will be skipped regardless of how many other particles are in the decay.
      So, for example, $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e \gamma$ will be skipped as well}. %TP: I don't think I explain it clearly... it should probably be redone.
...
At present only the following debug messages are present in the code:
\begin{itemize}
...
\begin{itemize}
  ...
  \item {\tt Photos::setMeCorrectionWtForScalar(bool corr)} \\
         A switch for complete effects of the matrix element (in scalar to two scalar decays) {\it  installed in {\tt PHOTOS 3.3}. {ZbW \bf At present tests are missing.} }
  \item {\tt Photos::setMeCorrectionWtForW(bool corr)} \\
        ...
        This option will be elaborated further. {\bf ZbW: I do not understand restriction. TP: I don't remember it as well. I only recall we had some problems with W. Needs to be investiagted.}
