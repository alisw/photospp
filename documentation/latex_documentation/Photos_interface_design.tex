...
public in year 2005 and multi-photon radiation was 
...
construction is left in  {\tt FORTRAN}. It does not feature the presented 
above options yet.
...
{\tt HepMC} versions 2.06, 2.05  and 2.03 were used  in final tests of our 
distribution. 
...
options for conventions of event record filling.
  \begin{itemize} 
    ...
    \item  If there is  4-momentum non-conservation\footnote{For details see 
           ...
           {\tt PHOTOS} will not be invoked too.  Special kinematic correcting
...
\item {\bf src/photosFortranInterfaces/ } -  interface to {\tt PHOTOS FORTRAN} routines and common blocks. \\
  Files:
    ...
    \item { \bf PH\_HEPEVT\_interface } - contains a wrapper for accessing
...
The main interface library requires that {\tt HepMC} \cite{Dobbs:2001ck} (version 2.03 or later)  has been installed
and its location has been provided during the configuration step. This is sufficient to compile the interface and to run the simple, stand-alone example.
...
\begin{itemize}
  ...
  \item add {\tt <PhotosLocation>/lib/libPhotosCxxInterface.a} at the linking step of your project.
\end{itemize}
...
\begin{itemize}
  ...
  \item {\tt Photos::setMomentumConservationThreshold(double momentum\_{}conservation\_{}threshold)} \\
        Default is 0.1 (in {\tt MEV/GEV}, depending
on units used by {\tt HepMC}). If larger energy-momentum non conservation
        is found then in  the vertex, photon generation  is skipped. 
        At present, for evaluation of non conservation  standard method of {\tt HepMC}
        is used.
...
%nd-some of the {\tt} words are going over the edge of the line (have
%a look a this page in the pdf file).  This should be fixed, but I
