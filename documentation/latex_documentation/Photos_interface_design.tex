...
in publications as well as in the code, now in {\tt C++}. \\
  ...
  \item {\bf examples/ } - examples of different {\tt PHOTOS} {\tt C++} Interface uses.
	...
	generated by {\tt PYTHIA 8.1} and processed by {\tt PHOTOS}. The analysis is done using {\tt MC-TESTER}, initialization for emission of lepton pairs
is demonstrated.
...
\item[PHOKEY] keys and parameters controlling the algorithm options.
	...
	\item[INTERF] \textit{bool} switch for interference, the matrix element weight.
...
only the {\tt HEPEVT} example will be provided. Otherwise, the {\tt /examples} directory will 
contain the executable files for {\it all} examples which can be compiled and linked, this may be of limited interest for some users.
...
directory, see section \ref{sec:autotools} for details. To set the appropriate paths, the scripts 
...
To activate this set of autotool\cite{autotools}-based installation scripts,
