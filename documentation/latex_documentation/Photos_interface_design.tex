...
It was rather simple to completely rewrite {\tt PHOTOS} to
C++ completely. To profit from numerical tests, the numerical core of {\tt PHOTOS}
...
C++ part of the code searches the whole event for
...
the event record of {\tt FORTRAN} as a struct type definition.
...
A description of the interface to the remaining, essentially plain {\tt C}, parts of the code is
...
The choice of splitting the source code into three main modules
 allows the separation of the {\tt C}  code of the numerical algorithm from the abstract C++ interface
...
\begin{itemize}
  \item {\bf {\tt photos-C} }\\
       ...
       to the {\tt C} numerical library of {\tt PHOTOS}. In particular,
...
in publications as well as in the code, now in C++. \\
  Files:
  \begin{itemize}
    \item { \bf struct\_decl.h } - contains declaration of structures and functions defined in other files within this folder. %TP: I just realized we don't have photos-C.h file. We can rename this file to photos-C.h and remove this entry from here
                                   {\tt Photos} class uses these structs to set several options and for internal variables such as weights, angles, four momenta of internal use.
    \item { \bf photos-C.cxx } - core functionality of {\tt PHOTOS}
    ...
    \item { \bf forZ-MEc.cxx } - routines to calculate matrix element in Z decay. 
                                 Note distinct programming style than the rest of code in {\tt photos-C} directory.
%TP: what does 'own programming style' mean?
...
\label{sec:extensibility}
 The purpose of the C++ interface to {\tt PHOTOS} algorithm rewritten to C is to enable 
...
for the implementation of channel dependent matrix elements. This could   
be  integrated all together into the C++ version of {\tt PHOTOS}. This completed by now task,
was not urgent.
The gain of precision was 
not that substantial, already earlier precision was quite good. 
...
but in declaration of C structs used internally in {\tt PHOTOS} algorithm.
...
structs of C  based on {\tt HEPEVT}: {\tt pho} and {\tt hep}, 
...
(see Section \ref{sect:F77fill}) for $W$ and $Z$ decays. It is also available for the two body decays of scalar 
into scalars. Then, exact mean exact with respect to scalar QED only. 
...
event records is now rewritten into C++ and uses the {\tt HepMC} event record 
...
No more  parts of the algorithm are left in {\tt FORTRAN}.
 This remaining {\tt FORTRAN} part of the code is for the optional interface to {\tt HEPEVT} of {\tt FORTRAN}. 
...
\section{Appendix: Interface to internal C part of {\tt PHOTOS}}
...
the  C++ definition style is adapted, small letters are for structs and variable names.
