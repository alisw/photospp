...
\footnote{An up-to-date version of the code described in this paper is
...
moving {\tt PHOTOS} to a C++ environment, allowing the use of event records such as {\tt HepMC},
...
Respectively, they describe the interface to the old part of the code, which has been rewritten from {\tt FORTRAN} to C,
...
construction has been also rewritten from {\tt FORTRAN} to C. It has been extended
over the last few years and features the options presented above.
...
by the core of the {\tt PHOTOS} code,
...
between vertices represent particles.  Fortunately, in both {\tt
...
\begin{itemize}
  ...
  \item \textbf{Status codes:} only information on whether a given particle is decaying (status 2) or stable (status 1).
...
%TP: I think this paragraph is not needed at all
