...
%nd-I don't understand the sentence below which begins ``This work,
%scattered over special...''
\item
...
For semi-inclusive cross sections, such as the total rate of Z decay for events
where the hardest photon energy does not exceeding 1 GeV in the Z rest frame, differences
between results from {\tt PHOTOS}, {\tt PHOTOS} with the matrix element correcting weight
...
On the other hand, for observables where two hard photons were requested
 ...
 30 \%. In this region of phase space {\tt PHOTOS}, even without the correcting
...
%nd-I've put the % in the table headers because I think otherwise it's
%ambigous if the statistical error we quote is absolute or relative.
...
%nd-the two routines below have no description of the return and input parameters
...
\begin{description}
\item[IPHEKL] enables/blocks emission in: $\pi^0 \rightarrow \gamma e^+ e^-$. \\
  Return type: \textit{int} \\
  Parameters: \textit{int}
\end{description}
