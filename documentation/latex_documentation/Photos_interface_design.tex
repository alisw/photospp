...
no. xx-xxx within IN2P3 through LAPP Annecy and 
...
They ccomparison booklets can be useful 
...
If a user is building a large simulation system she or he may want to avoid our 
configuration infrastructure and load  the libraries only. 
For that purpose our stand-alone 
example {\bf ZBW: localized in ...} is a good starting point.
...
branches have been introduced and are presented in the Appendix. \\ \\ 
...
\begin{itemize}
 ...
 \item {\tt Photos::suppressBrem(0, motherID)} {\bf ZBW: ForDecay ??} \hfill \\
...
\emph{If the decays $\tau^- \rightarrow \nu_\tau e^- \bar \nu_e$ or
	  ...
	  (which can be zero if no restriction is requested {\bf ZBW: ??}) and the mother PDGID.} \\ \\
...
At present only the debug codes listed below are used: {\bf ZBW: need better explanation, out of context.}
\begin{itemize}
...
sufficiently hard photons from allgenerated
...
process to the $100 \rightarrow Y t \bar t$ process, {\bf ZBW: is it still true that Y is among decay products?}
