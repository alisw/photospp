...
\item {\bf src/utilities/ } - source code for utilities.\\
  ...
  \item { \bf PhotosRandom} - random number generator of Ref.~\cite{} taken from {\tt PHOTOS FORTRAN} and rewritten to {\tt C++}.
...
\item[PHOTOS\_MAKE\_C]  like {\tt PHOTOS\_MAKE} of {\tt FORTRAN} part of interface, but now in C. \\
  ...
  Parameters:
  \begin{enumerate}
    \item \textit {int id} ID of particle from which {\tt PHOTOS} starts processing. In the {\tt C++} case the importance of this parameter is limited as only one branch is in {\tt HEPEVT} at a time.
  \end{enumerate}
\end{description}
...
bremsstrahlung is prevented for vertices involving gluons or quarks 
...
\begin{itemize}
 ...
 \item {\tt Photos::suppressBremForDecay(0, motherID)} \hfill \\
	   ...
	   If   set  to zero all decay channels are skpied and not only of a given
           multiplicity. {\bf ZbW: I do not understand role of first paprameter equal 0, it is implicityl given below. The above sentence may be incorrect.}
...
The matrix element used for pair emission  is easy to improve. It is using four momenta of final state particles
explicitly. 
