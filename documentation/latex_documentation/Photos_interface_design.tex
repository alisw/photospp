...
\item {\bf src/utilities/ } - source code for utilities.\\
  ...
  \item {\bf examples/ } - examples of different {\tt PHOTOS} C++ Interface uses.
	...
	the The decay chain is processed by {\tt PHOTOS} and then analysed with {\tt MC-TESTER}.
    \end{itemize}   
...
\item
Methods deviced to check the content of HepMC as used by {\tt PHOTOS} are
...
Experince gained from many years of developing and maintaining the algorithm
...
at 0.2 \% level. 
On the other hand, for observalbes where two hard photons were requested
...
This plot have been created using a custom {\tt UserTreeAnalysis} of {\tt MC-TESTER}.
...
where zero, one or at least two photons of energy above 1 GeV accompanying $\mu^+\mu^-$ pair,
...
They are of limited but noneteless  of  some physics interest as well.
...
\begin{itemize}
  \item {\tt Photos::setSeed(int iseed1, int iseed2)} \\
        Set the  seed values for our copy of random number geneator {\tt RANMAR} \cite{ranmar:1990}.
